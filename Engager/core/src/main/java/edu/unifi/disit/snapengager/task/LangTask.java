...
@EnableScheduling
@Component
public class LangTask implements SchedulingConfigurer {

	private static final Logger logger = LogManager.getLogger();

	@Value("${lang.task.cron}")
	...
	public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
		taskRegistrar.addTriggerTask(new Runnable() {
			...
			public void run() {
				try {
					logger.debug("-----------------------------------------------start execution for LANGUAGE-----------------------------------------------");
					myTask();
				} catch (Exception e) {
			...
			public Date nextExecutionTime(TriggerContext triggerContext) {
				...
				logger.debug("-----------------------------------------------next execution for LANGUAGE will be fired at:{}-----------------------------------------------", nextExec);
				return nextExec;
			}
		});
	}
	...
	private void myTask() throws CredentialsException, IOException, UserprofileException {
		...
		for (Data language : dataservice.getLangData(lang)) {
			// check if the user has enable the assistance
			if ((assistanceEnabled.get(language.getUsername()) != null) && (assistanceEnabled.get(language.getUsername()))) {
				Userprofile up = upservice.get(language.getUsername(), lang);
				if (up == null)
					up = new Userprofile(language.getUsername());
				up.setLanguage(language.getVariableValue());
