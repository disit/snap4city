...
def PrintVmInfo(vm, content, vchtime, interval, perf_dict, ):
    ...
    print('"Server Name":"{}",'.format(summary.config.name))
    print('"Description":"{}",'.format(summary.config.annotation))
    ...
    try:
        ...
        pass
    print('],')
    print('"Virtual NIC(s)":["{}"'.format(network_list[0]))
    if len(network_list) > 1:
        network_list.pop(0)
        for each_vnic in network_list:
            print(',"{}"'.format(each_vnic))
    print('],')
    print('"[VM] Limits":"CPU: {}, Memory: {}",'.format(vmcpulimit, vmmemlimit))
    print('"[VM] Reservations":"CPU: {}, Memory: {}",'.format(vmcpures, vmmemres))
    print('"[VM] Number of vCPUs":"{}",'.format(summary.config.numCpu))
    print('"[VM] CPU Ready":"Average {:.1f} %, Maximum {:.1f} %",'.format((cpuReady / 20000 * 100),
                                                                                       ((float(max(
                                                                                           statCpuReady[0].value[
                                                                                               0].value)) / 20000 * 100))))
    print('"[VM] CPU (%)":"{:.2f} %",'.format(cpuUsage))
    print('"[VM] Memory":"{} MB ({:.2f} GB)",'.format(summary.config.memorySizeMB, (float(summary.config.memorySizeMB) / 1024)))
    print('"[VM] Memory Shared":"{:.2f} %, {:.2f} MB",'.format(
        ((memoryShared / summary.config.memorySizeMB) * 100), memoryShared))
    print('"[VM] Memory Balloon":"{:.2f} %, {:.2f} MB",'.format(
        ((memoryBalloon / summary.config.memorySizeMB) * 100), memoryBalloon))
    print('"[VM] Memory Swapped":"{:.2f} %, {:.2f} MB",'.format(
        ((memorySwapped / summary.config.memorySizeMB) * 100), memorySwapped))
    print('"[VM] Memory Active":"{:.2f} %, {:.2f} MB",'.format(
        ((memoryActive / summary.config.memorySizeMB) * 100), memoryActive))
    print('"[VM] Datastore Average IO":"Read: {:.2f} IOPS, Write: {:.2f} IOPS",'.format(DatastoreIoRead,
                                                                                          DatastoreIoWrite))
    print('"[VM] Datastore Average Latency":"Read: {:.2f} ms, Write: {:.2f} ms",'.format(DatastoreLatRead,
                                                                                      DatastoreLatWrite))
    ...
    print('"[Host] Memory Usage":"Used: {:.2f} GB, Total: {:.2f} GB"}}'.format(
...
def main():
    ...
    try:
        ...
        vm = getVMFromUUID(search_index, args.vm)
        PrintVmInfo(vm.summary.vm, content, vchtime, args.interval, perf_dict)
