...
public class ClusterGen {
    ...
    public boolean performDBSCAN(double eps, int minLns) {
        ...
        for (int i = 0; i < m_nTotalLineSegments; i++) {//m_componentIdArray.set(i,UNCLASSIFIED);
...
//		m_projectionPoint = new CMDPoint();
    ...
    boolean constructLineSegmentCluster() {
        ...
        for (int i = 0; i < m_currComponentId; i++) {
            ...
            if (clusterEntry.nTrajectories >= m_minLnsParam) //if(m_lineSegmentClusters[i].nTrajectories >= m_minLnsParam)
            {
	...
	private void RegisterAndUpdateLineSegmentCluster(int componentId, int lineSegmentId) {
        ...
        if (!clusterEntry.trajectoryIdList.contains(trajectoryId)) //if (std::find(clusterEntry.trajectoryIdList.begin(), clusterEntry->trajectoryIdList.end(), trajectoryId) == clusterEntry.trajectoryIdList.end())
        {
    ...
    private boolean storeLineSegmentCluster() {
        ...
        for (int i = 0; i < m_currComponentId; i++) {
            ...
            if (m_lineSegmentClusters[i].enabled) {
                ...
                // Daniele: set the trajectories id list for this cluster
                pClusterItem.setTrajectoriesIdList(m_lineSegmentClusters[i].trajectoryIdList);
    ...
    private double subComputeDistanceBetweenTwoLineSegments(CMDPoint startPoint1,
        ...
        if (length1 > length2) {
            ...
            if (m_coefficient < 0.5) {
                parDistance1 = measureDistanceFromPointToPoint(startPoint1, m_projectionPoint);
            } else {
                parDistance1 = measureDistanceFromPointToPoint(endPoint1, m_projectionPoint);
            }
            
            perDistance2 = measureDistanceFromPointToLineSegment(startPoint1, endPoint1, endPoint2);
            if (m_coefficient < 0.5) {
                parDistance2 = measureDistanceFromPointToPoint(startPoint1, m_projectionPoint);
            } else {
                parDistance2 = measureDistanceFromPointToPoint(endPoint1, m_projectionPoint);
            }
        } else {
            perDistance1 = measureDistanceFromPointToLineSegment(startPoint2, endPoint2, startPoint1);
            if (m_coefficient < 0.5) {
                parDistance1 = measureDistanceFromPointToPoint(startPoint2, m_projectionPoint);
            } else {
                parDistance1 = measureDistanceFromPointToPoint(endPoint2, m_projectionPoint);
            }
            
            perDistance2 = measureDistanceFromPointToLineSegment(startPoint2, endPoint2, endPoint1);
            if (m_coefficient < 0.5) {
                parDistance2 = measureDistanceFromPointToPoint(startPoint2, m_projectionPoint);
            } else {
                parDistance2 = measureDistanceFromPointToPoint(endPoint2, m_projectionPoint);
            }
