        ...
        var changeHandler = function(evt) {
            ...
            if (node.type === undefined) {
                ...
                if (this.value === 'local') {
                    ...
                    $("."+className+"-"+this.value).closest(".node-diff-node-entry").addClass("node-diff-select-remote");o
                }
            } else {
...
RED.clipboard = (function() {
          ...
          var flowToExport =  [];
                ...
                for(var i= 0;i<linkinNodes.length;i++){
                 ...
                 for(var i=0;i<flowToExport.length;i++){
                      nodesToExport =  nodesToExport.concat(RED.nodes.filterNodes({z:flowToExport[i]}));
                    

                 }

                //il primo flusso  non ha parent flow per essere importato  sia in modalita "current fLow" che in modalità "parent flow"
    ...
    function cloudExportNodes() {
        ...
        var natureUrl = "https://servicemap.disit.org/WebAppGrafo/sparql?query=select%20distinct%20substr(str(?m),37)%20as%20?macroClass%20{%20?m%20rdfs:subClassOf%20km4c:Service.%20filter%20exists%20{?c%20rdfs:subClassOf%20?m}%20}%20order%20by%20?m&format=json";
        var subnatureUrl = "https://servicemap.disit.org/WebAppGrafo/sparql?query=%20select%20distinct%20substr(str(?m),37)%20as%20?macroClass%20substr(str(?c),37)%20as%20?subClass%20{%20?m%20rdfs:subClassOf%20km4c:Service.%20?c%20rdfs:subClassOf+%20?m.%20}%20order%20by%20?m%20?c&format=json";
