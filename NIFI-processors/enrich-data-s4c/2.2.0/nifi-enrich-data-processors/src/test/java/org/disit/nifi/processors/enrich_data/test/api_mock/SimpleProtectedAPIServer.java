...
//	public class ProtectionHandler extends AbstractHandler{
	public class ProtectionHandler extends Handler.Abstract {
...
//		}
		...
		private com.github.scribejava.core.model.Response verifyAccessToken( String accessToken ) throws InterruptedException, ExecutionException, IOException {
...
//				System.out.println( "Verification request OAuth parameters: " + oaRequest.getOauthParameters().toString() );
		...
		private void setResponse( Response response , Callback callback , int stausCode , String payload ) {
		...
		private void setResponse( Response response , Callback callback , int statusCode , String payload , MediaType contentType) {
		...
		public boolean handle(Request request, Response response, Callback callback) throws Exception {
			System.out.println( "HANDLING PROTECTED" );
			HttpFields requestHeaders = request.getHeaders().asImmutable();
			if( !requestHeaders.contains( HttpHeader.AUTHORIZATION.asString() ) ) {
				setResponse( response , callback , HttpStatus.BAD_REQUEST_400 , "{\"error\":\"No 'Authorization' header provided.\"}" );
				return true;
			}
			
			String authHeaderValue = requestHeaders.get( HttpHeader.AUTHORIZATION.asString() );
			if( !authHeaderValue.startsWith( "Bearer " ) && !authHeaderValue.startsWith( "bearer " ) ) {
				setResponse( response , callback , HttpStatus.BAD_REQUEST_400 , 
					"{\"error\":\"Expecting 'Authorization' header value to start with 'bearer '.\"}"
				);
			}
			...
			JsonObject rObj;
			try( com.github.scribejava.core.model.Response oaResponse = verifyAccessToken( accessTokenStr ) ) {
				rObj = new JsonObject();
				rObj.addProperty( "verified-access-token", accessTokenStr );
				rObj.addProperty( "keycloak-response-code" , oaResponse.getCode() );
				System.out.println( oaResponse.getBody() );
				rObj.add( "keycloak-response-body", JsonParser.parseString( oaResponse.getBody() ) );
				
				if( this.protectedHandler == null ) {
					setResponse( response , callback , HttpStatus.OK_200 , rObj.toString() , MediaType.JSON_UTF_8 );
					return true;
				} 
				
				if( oaResponse.getCode() == HttpServletResponse.SC_OK ) {
					protectedHandler.handle( request , response , callback );
					return true;
				} else {
					setResponse( response , callback , oaResponse.getCode() , rObj.toString() , MediaType.JSON_UTF_8 );
					return true;
				}
			} catch (InterruptedException | ExecutionException e) {
				setResponse( response , callback , HttpStatus.INTERNAL_SERVER_ERROR_500 , 
					"{\"error\":\"Exception during token verification request.\"}" 
				);
				return true;
			}
		}
