...
public class KPIDataDAOImpl implements KPIDataDAOCustom {

	@PersistenceContext
	EntityManager entityManager;

	@Override
	public Page<KPIData> findKPIDataFilteredPage(String username, String highLevelType, String searchKey,
			Pageable pageable) {
		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		CriteriaQuery<KPIData> criteria = cb.createQuery(KPIData.class);
		Root<KPIData> dataRoot = criteria.from(KPIData.class);
		...
		if (searchKey != null) {
			...
			filterPredicates.add(cb.like(dataRoot.<String>get("nature"), "%" + searchKey + "%"));
			filterPredicates.add(cb.like(dataRoot.<String>get("subNature"), "%" + searchKey + "%"));
			filterPredicates.add(cb.like(dataRoot.<String>get("valueName"), "%" + searchKey + "%"));
			filterPredicates.add(cb.like(dataRoot.<String>get("valueType"), "%" + searchKey + "%"));
			filterPredicates.add(cb.like(dataRoot.<String>get("dataType"), "%" + searchKey + "%"));
			filterPredicates.add(cb.like(dataRoot.<String>get("lastValue"), "%" + searchKey + "%"));
		...
		List<KPIData> listKpiData = entityManager.createQuery(criteria).setFirstResult(pageable.getPageNumber()*pageable.getPageSize())
	...
	@Override
	public List<KPIData> findKPIDataFilteredList(String username, String highLevelType, String searchKey) {
		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		CriteriaQuery<KPIData> criteria = cb.createQuery(KPIData.class);
		Root<KPIData> dataRoot = criteria.from(KPIData.class);
