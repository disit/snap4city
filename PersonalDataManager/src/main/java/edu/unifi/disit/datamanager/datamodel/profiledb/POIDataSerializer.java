	...
	public void serialize(POIData poiData, JsonGenerator jgen, SerializerProvider provider) throws IOException {
		...
		try {
			...
			if (!linestring.equals("LINESTRING()")) {
				jgen.writeStringField("wktGeometry", linestring);
			}
			...
			if (!poiData.getListKPIValue().isEmpty()) {
				...
				for (KPIValue kpiValue : poiData.getListKPIValue()) {
					...
					if (kpiValue.getLatitude() != null) {
						jgen.writeStringField("latitude", kpiValue.getLatitude());
					}
					if (kpiValue.getLongitude() != null) {
						jgen.writeStringField("longitude", kpiValue.getLongitude());
					}
			...
			if (!poiData.getListKPIValue().isEmpty()) {
				...
				for (KPIValue kpiValue : poiData.getListKPIValue()) {
					if (kpiValue.getLongitude() != null && !kpiValue.getLongitude().equals("") && kpiValue.getLatitude() != null
						...
						jgen.writeStringField("type", "Point");
						jgen.writeArrayFieldStart("coordinates");
						jgen.writeNumber(kpiValue.getLongitude());
						jgen.writeNumber(kpiValue.getLatitude());
						jgen.writeEndArray();
						jgen.writeEndObject();
						jgen.writeObjectFieldStart("properties");
						jgen.writeStringField("value", kpiValue.getValue());
			...
			logger.debug("Invoked POIData on a KPI without lat/long information");
