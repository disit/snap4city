...
module.exports = function (RED) {
    ...
    function ServiceSearchWithinCircle(config) {
        ...
        node.on('input', function (msg) {
            var kb = (msg.payload.knowledgebase ? msg.payload.knowledgebase : "servicemap");
            var path = "ServiceMap";
            if (kb == "servicemap") {
                path = "WebAppGrafo";
            }
            var uri = "http://" + kb + ".km4city.org/" + path + "/api/v1/";
            ...
            xmlHttp.open("GET", encodeURI(uri + "?selection=" + latitude + ";" + longitude + "&categories=" + categories + "&maxResults=" + maxResults + "&maxDists=" + maxDists + "&format=json" + "&lang=" + language + "&geometry=true&fullCount=false" + (typeof uid != "undefined" && uid != "" ? "&uid=" + uid : "") + "&appID=iotapp"), false); // false for synchronous request
            xmlHttp.send(null);
