...
module.exports = function (RED) {
    ...
    function SaveMyData(config) {
        ...
        node.on('input', function (msg) {
            ...
            var motivation = (msg.payload.motivation ? msg.payload.motivation : config.motivation);
            var accessToken = retrieveAccessToken();
            var XMLHttpRequest = require("xmlhttprequest").XMLHttpRequest;
            var xmlHttp = new XMLHttpRequest();
            if (checkConsent) {
                console.log(encodeURI(uri + "?accessToken=" + accessToken));
                xmlHttp.open("POST", encodeURI(uri + "?sourceRequest=iotapp&accessToken=" + accessToken), false);
                ...
                eventLog(inPayload, msg, config, "Node-Red", "MyData", uri, "TX");
            } else {
                msg.payload = JSON.parse("{'failure': 'You don't have the permission'}");
                throw "You don't have the permission"
            }
            node.send(msg);
        });
    }
