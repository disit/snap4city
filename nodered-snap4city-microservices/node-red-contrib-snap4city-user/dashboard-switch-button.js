...
module.exports = function (RED) {
    ...
    function SwitchButton(config) {
        ...
        node.httpServerPostCallback = function (req, res) {
            ...
            console.log(node.getNow() + " - New HTTP request received from switch-button node " + node.name + ": " + msg);
        ...
        node.httpServerPostErrorHandler = function (e) {
            console.log(node.getNow() + " - Error managing HTTP request for switch-button node " + node.name + ": " + e);
        };
        ...
        console.log(node.getNow() + " - httpRoot parameter for switch-button node " + node.name + ": " + RED.settings.httpRoot);
        ...
        request({
            ...
            function (error, response, body) {
                if (error === null) {
                    console.log(node.getNow() + " - switch-button node " + node.name + " sent ADD_EMITTER request to Dashboard manager - Response: " + JSON.stringify(response));
                } else {
                    console.log(node.getNow() + " - switch-button node " + node.name + " sent ADD_EMITTER request to Dashboard manager BUT GOT ERROR - Response: " + JSON.stringify(response) + " - Error: " + JSON.stringify(error));
                }
            }
        );
        ...
        node.delEmitter = function () {
            ...
            request({
                ...
                function (error, response, body) {
                    console.log(node.getNow() + " - switch-button node " + node.name + " sent DEL_EMITTER request to Dashboard manager - Response: " + response + " - Error: " + error);
                });
        };
        ...
        node.nodeClosingDone = function () {
            console.log(node.getNow() + " - switch-button node " + node.name + " has been closed");
        };

        node.on('close', function (removed, nodeClosingDone) {
            if (removed) {
                // Cancellazione nodo
                console.log(node.getNow() + " - switch-button node " + node.name + " is being removed from flow");
                node.delEmitter();
            } else {
                // Riavvio nodo
                console.log(node.getNow() + " - switch-button node " + node.name + " is being rebooted");
            }
            nodeClosingDone();
