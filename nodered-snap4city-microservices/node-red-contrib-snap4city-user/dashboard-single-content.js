...
module.exports = function (RED) {
    ...
    function SingleContentNode(config) {
        ...
        node.openWsConn = function () {
            ...
            console.log(node.getNow() + " - Single-content node " + node.name + "IS GOING TO CONNECT WS");
        ...
        node.on('input', function (msg) {
            console.log(node.getNow() + " - Flow input received for single-content node " + node.name + ": " + msg);
            ...
            if ((new Date().getTime() - node.wsStart) > parseInt(RED.settings.wsReconnectTimeout) * 1000) {
                ...
                console.log(node.getNow() + " - single-content node " + node.name + " is reconnetting to open WebSocket");
                timeout = 1000;
            }
            ...
            setTimeout(function () {
                try {
                    ...
                    console.log(node.getNow() + " - Error sending data to WebSocket for single-content node " + node.name + ": " + JSON.stringify(e));
                }
            }, timeout);
            ...
            switch (response.msgType) {
                case "AddEditMetric":
                    if (response.result === "Ok") {
                        console.log(node.getNow() + " - WebSocket server correctly added/edited metric type for single-content node " + node.name + ": " + response.result);
                    } else {
                        //TBD - CASI NEGATIVI DA FARE
                        console.log(node.getNow() + " - WebSocket server could not add/edit metric type for single-content node " + node.name + ": " + response.result);
                    }
                    break;

                case "DelMetric":
                    if (response.result === "Ok") {
                        console.log(node.getNow() + " - WebSocket server correctly deleted metric type for single-content node " + node.name + ": " + response.result);
                    } else {
                        //TBD - CASI NEGATIVI DA FARE
                        console.log(node.getNow() + " - WebSocket server could not delete metric type for single-content node " + node.name + ": " + response.result);
                    }
                    console.log(node.getNow() + " - Closing webSocket server for single-content node " + node.name);
                    node.ws.close();
                    break;
        ...
        this.delMetric = function () {
            console.log(node.getNow() + " - Deleting metric via webSocket for single-content node " + node.name);
            var newMsg = {
                msgType: "DelMetric",
                metricName: encodeURIComponent(node.metricName),
                metricType: node.metricType,
            ...
            try {
                ...
                console.log(node.getNow() + " - Error deleting metric via webSocket for single-content node " + node.name + ": " + e);
            }
        };

        this.openWs = function (e) {
            console.log(node.getNow() + " - single-content node " + node.name + " is trying to open WebSocket");
            try {
                node.status({
                    fill: "yellow",
                    shape: "dot",
                ...
                console.log(node.getNow() + " - single-content node " + node.name + " could not open WebSocket");
                node.status({
                    fill: "red",
                    shape: "ring",
        ...
        this.wsClosed = function (e) {
            console.log(node.getNow() + " - single-content node " + node.name + " closed WebSocket");
            node.status({
                fill: "red",
                shape: "ring",
            ...
            if (RED.settings.wsServerRetryActive === 'yes') {
                console.log(node.getNow() + " - single-content node " + node.name + " will try to reconnect to WebSocket in " + parseInt(RED.settings.wsServerRetryTime) + "s");
                setTimeout(node.openWs, parseInt(RED.settings.wsServerRetryTime) * 1000);
            }
        };

        this.wsError = function (e) {
            console.log(node.getNow() + " - single-content node " + node.name + " got WebSocket error: " + e);
        };
        ...
        try {
            ...
            console.log(node.getNow() + " - single-content node " + node.name + " got main exception connecting to WebSocket");
        }
        ...
        this.nodeClosingDone = function () {
            console.log(node.getNow() + " - single-content node " + node.name + " has been closed");
        };

        this.on('close', function (removed, nodeClosingDone) {
            if (removed) {
                // Cancellazione nodo
                console.log(node.getNow() + " - single-content node " + node.name + " is being removed from flow");
                node.delMetric();
            } else {
                // Riavvio nodo
                console.log(node.getNow() + " - single-content node " + node.name + " is being rebooted");
            }
            node.ws.close();
            nodeClosingDone();
        });
