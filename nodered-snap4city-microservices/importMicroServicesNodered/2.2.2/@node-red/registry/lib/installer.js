...
function installMicroservice(url) {
    ...
    return new Promise((resolve, reject) => {
        var installUrl = url;
        var installName = url.substring(url.lastIndexOf("/") + 1);
        var module = url.substring(url.lastIndexOf("/") + 1, url.lastIndexOf("."));
        if (module.lastIndexOf(".") != -1) {
            module = module.substring(0, module.lastIndexOf("."));
        }
        var installDir = (settings.userDir || process.env.NODE_RED_HOME || ".") + "/node_modules";
        var fatherChild = child_process.execFile("mkdir", ["-p", "node_modules"], {
                cwd: (settings.userDir || process.env.NODE_RED_HOME || ".")
            },
            function (err, stdin, stdout) {
                console.log(err);
                if (err) {
                    log.warn(log._("server.install.mkdir-failed", {
                        name: module
                    }));
                    reject(new Error(log._("server.install.mkdir-failed")));
                } else {
                    var child = child_process.execFile("wget", [installUrl], {
                            cwd: installDir
                        },
                        function (err, stdin, stdout) {
                            ...
                            if (err) {
                                ...
                                e = new Error("Problem on download zip");
                                e.code = 502;
                                reject(e);
                            } else {
                                var subChild = child_process.execFile("unzip", ["-o", installName], {
                                        cwd: installDir
                                    },
                                    function (err, stdin, stdout) {
                                        console.log(err);
                                        if (err) {
                                            log.warn(log._("server.install.unzip-failed", {
                                                name: module
                                            }));
                                            reject(new Error(log._("server.install.unzip-failed")));
                                        } else {
                                            var subSubChild = child_process.execFile("rm", [installName], {
                                                    cwd: installDir
                                                },
                                                function (err, stdin, stdout) {
                                                    //NOT NOTIFIED
                                            ...
                                            resolve(require("./index").addMicroservice(module).then(reportAddedModules));
                                        }
                                    }
                                );
                            }
