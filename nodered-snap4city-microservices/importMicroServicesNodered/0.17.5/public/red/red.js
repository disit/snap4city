    ...
    function save(skipValidation, force) {
        if (!$("#btn-deploy").hasClass("disabled")) {
            ...
            $.ajax({
                ...
                $("#btn-deploy").removeClass("disabled");
                if (xhr.status === 401) {
                    RED.notify(RED._("deploy.deployFailed", {
...
RED.diff = (function () {
    ...
    function createNodePropertiesTable(def, node, localNodeObj, remoteNodeObj) {
        ...
        var properties = Object.keys(node).filter(function (p) {
            return p != 'z' && p != 'wires' && p !== 'x' && p !== 'y' && p !== 'id' && p !== 'type' && (!def.defaults || !def.defaults.hasOwnProperty(p))
        });
        if (def.defaults) {
...
RED.clipboard = (function () {
    ...
    function getAccessToken() {
        ...
        $.get("/data/refresh_token", function (refreshToken) {
                console.log("success:" + refreshToken);
            }).done(function (refreshToken) {
                console.log("success single res" + refreshToken);
    ...
    function setupDialogs() {
        dialog = $('<div id="clipboard-dialog" class="hide node-red-dialog"><form class="dialog-form form-horizontal"></form></div>')
            ...
            .dialog({
                ...
                buttons: [{
                    ...
                    /* s4c init */
                    {
                        ...
                        click: function () {
                            ...
                            var cloudMessage = {
                                ...
                                // "url":$("#node-cloud-export-url").val(),
                                //"method":$("#node-cloud-export-method").val(),
                                //  "help":$("#node-cloud-export-help").val()
                                "data": cloudData
                            ...
                            // console.log(RED.settings.user.username);

                            console.log("messaggio s4c");
                            // console.log(JSON.stringify(cloudMessage));
                            console.log(cloudMessage);

                            var cloudDialog = $(this);


                            var jqxhr = $.post("https://processloader.snap4city.org/processloader/api/upload.php", JSON.stringify(cloudMessage), function (data) {
                                ...
                                .done(function (data) {
                                    ...
                                    //console.logoudDialog = $( this );( "second success" + data.result);
                                    //console.log( "second success" + data.message);
                    ...
                    /* microservices init */
                    {
                        id: "clipboard-dialog-microservice",
                        class: "primary palette-module-install-confirm-button-install",
                        text: RED._("palette.editor.confirm.button.install"),
                        click: function () {
                            //installExternalMicroservice
                            //var version = $(this).data('version');
                            var shade = $(this).data('shade');
                            ...
                            var content_clipboard = $("#clipboard-import").val();
                            content_clipboard = content_clipboard.replace(/"/g, "");
                            ///
                            $.ajax({
                                url: "microservices",
                                type: "POST",
                                data: JSON.stringify({
                                    url: content_clipboard
                                }),
                                contentType: "application/json; charset=utf-8"
                            }).done(function (data, textStatus, xhr) {
                                ...
                                //alert(xhr.status);
                                if (xhr.status !== 200) {
                                    RED.notify(RED._("Import Failed", {
                                        message: err
                                    }), "error");
                                }
                                //
                    ...
                    /* microservices end */
                    /* damicroservices init */
                    {
                        id: "clipboard-dialog-damicroservice",
                        class: "primary palette-module-install-confirm-button-install",
                        text: RED._("palette.editor.confirm.button.install"),
                        click: function () {
                            //installExternalMicroservice
        ...
        exportNodesDialog =
            ...
            '</div>';
        /* s4c init */
        cloudExportNodesDialog = exportNodesDialog +
            '<div class="form-row" style="color:red; font-size:16px;">' +
            '<p id="node-cloud-export-error-msg" ></p>' +
            '</div>' +
            '<div class="form-row">' +
            '<label for="node-cloud-export-name" >Title *</label>' +
            '<input type="text" id="node-cloud-export-name" placeholder="" >' +
            '</div>' +
            '<div class="form-row">' +
            '<label for="node-cloud-export-nature" >Nature *</label>' +
            '<select id="cloud-data-export-nature">' +
            '<option value="" >Select a value</option>' +
            '</select>' +

            '</div>' +
            '<div class="form-row">' +
            '<label for="node-cloud-export-subnature" >Subnature *</label>' +
            '<select id="cloud-data-export-subnature">' +
            '<option data-nature="default" value="" >Select a value</option>' +
            '</select>' +
            '</div>' +
            '<div class="form-row">' +
            '<label for="node-cloud-export-desc" >Description *</label>' +
            '<textarea rows="4" id="node-cloud-export-desc" placeholder="" style="width:70%"></textarea>' +
            '</div>';
        /* '<div class="form-row">'+
             '<label for="node-cloud-export-type-app" >Type app *</label>'+
             '<select id="node-cloud-export-type-app"  >'+
                 '<option value="library">Library</option>'+
                 '<option value="block">Block</option>'+
             '</select>'+
         '</div>'+
         '<div class="form-row">'+
             '<label for="node-cloud-export-url" >Url</label>'+
             '<input type="text" id="node-cloud-export-url" placeholder="" >'+
         '</div>'+
         '<div class="form-row">'+
             '<label for="node-cloud-export-method" >Method</label>'+
             '<select id="node-cloud-export-method"  >'+
                 '<option value="GET">GET</option>'+
                 '<option value="POST">POST</option>'+
             '</select>'+
         '</div>' +
         '<div class="form-row">'+
             '<label for="node-cloud-export-help" >Help</label>'+
             '<textarea rows="4" id="node-cloud-export-help" placeholder="" style="width:70%"></textarea>'+
         '</div>';*/
        /* s4c end */

        exportSaveNodesDialog =
            '<div class="form-row">' +
            '<label style="width:auto;margin-right: 10px;" data-i18n="clipboard.export.copy"></label>' +
            '<span id="export-range-group" class="button-group">' +
            '<a id="export-range-selected" class="editor-button toggle" href="#" data-i18n="clipboard.export.selected"></a>' +
        ...
        cloudImportNodesDialog =
            '<div class="form-row" style="color:red; font-size:16px;">' +
            '<p id="node-cloud-import-error-msg" ></p>' +
            '</div>' +
            '<div class="form-row">' +
            '<ul id="cloudImportList" style="list-styl-type:none; display:block; height:200px; overflow-y:scroll;"></ul>' +
            '</div>' +
            '<div class="form-row">' +
            '<textarea style="resize: none; width: 100%; border-radius: 0px;font-family: monospace; font-size: 12px; background:#eee; padding-left: 0.5em; box-sizing:border-box;" id="clipboard-import" rows="5" placeholder="' +
            RED._("clipboard.pasteNodes") +
    ...
    function cloudImportNodes() {
        ...
        var jqxhr = $.get("https://processloader.snap4city.org/processloader/api/download.php?resource_type=IoTApp", function (data) {
                console.log("success:" + data);
            })
            .done(function (data) {
                ...
                if (data.result == "Ok") {
                    ...
                    //console.log(cloudImportListOptions);
                    ...
                    $('#cloudImportList a').click(function (evt) {
                        ...
                        var jqxhr = $.get("https://processloader.snap4city.org/processloader/api/download.php?id=" + id, function (singleResData) {
        ...
        /* microservices init */
        $("#clipboard-dialog-microservice").hide();
        /* microservices end */
        /* damicroservices init */
        $("#clipboard-dialog-damicroservice").hide();
        /* damicroservices end */
    ...
    ///////////////////////////////////////////////////////////////////////////////
    /* microservices init */
    function cloudImportMicroService() {
        if (disabled) {
            return;
        }
        dialogContainer.empty();
        ...
        dialogContainer.i18n();
        /* dovnload list of resources */
        $('#clipboard-import').hide();
        $('#import-tab').parent().hide();
        var jqxhr = $.get("https://processloader.snap4city.org/processloader/api/download.php?resource_type=MicroService", function (data) {
                //var jqxhr = $.get( "http://192.168.56.200/Sviluppo2/api/download.php?resource_type=Microservice",   function( data) {
                console.log("success:" + data);
            })
            .done(function (data) {
                data = JSON.parse(data);
                if (data.result == "Ok") {
                    if (data.hasOwnProperty('files')) {
                        ...
                        //$("#node-cloud-import-error-msg").html(""+data.message); 
                        $("#node-cloud-import-error-msg").html("" + "Not Microservices Founded");
                    }
                    //console.log(cloudImportListOptions);
                    $('#cloudImportList').append(cloudImportListOptions);
                    $('#cloudImportList a').click(function (evt) {
                        ...
                        var id = $(this).data('val');
                        var jqxhr = $.get("https://processloader.snap4city.org/processloader/api/download.php?id=" + id, function (singleResData) {
                                //var jqxhr = $.get( "http://192.168.56.200/Sviluppo2/api/microservices.php?get_resource="+id,  function( singleResData) {
                            }).done(function (singleResData) {
                                //var fs = require('fs');
                                singleResData = JSON.parse(singleResData);
                                console.log("success single res" + singleResData);
                                if (singleResData.result == "Ok") {
                                    if (singleResData.hasOwnProperty('file_link')) {
                                        ...
                                        //$('#clipboard-import').val(JSON.stringify(singleResData));
                                        $('#clipboard-import').val(JSON.stringify(singleResData['file_link']));
                                        $('#clipboard-import').trigger('keyup');
                                        $('#clipboard-import').hide();
                                    } else if (data.hasOwnProperty('message')) {
                                        $("#node-cloud-import-error-msg").html("" + singleResData.message);
        ...
        //COMANDO PER L'IMPORT
        $("#clipboard-dialog-ok").hide();
        $("#clipboard-dialog-microservice").show();
        /* s4c init */
        $("#cloud-dialog-copy").hide();
        /* s4c end */
        /* damicroservices init */
        ...
        $("#clipboard-dialog").parent().find(".ui-dialog-titlebar").html("Import External MicroService");
        $("#clipboard-dialog-ok").button("disable");
        $("#clipboard-import").keyup(validateImport);
        $("#clipboard-import").on('paste', function () {
            setTimeout(validateImport, 10)
    ...
    /* damicroservices init */
    function cloudImportDAMicroService() {
        if (disabled) {
            return;
        }
        dialogContainer.empty();
        ...
        $('#clipboard-import').hide();
        $('#import-tab').parent().hide();
        var jqxhr = $.get("https://processloader.snap4city.org/processloader/api/download.php?resource_type=DataAnalyticMicroService", function (data) {
                //var jqxhr = $.get( "http://192.168.56.200/Sviluppo2/api/download.php?resource_type=DataAnalyticMicroService",   function( data) {
                console.log("success:" + data);
            })
            .done(function (data) {
                data = JSON.parse(data);
                if (data.result == "Ok") {
                    if (data.hasOwnProperty('files')) {
                        ...
                        //$("#node-cloud-import-error-msg").html(""+data.message); 
                        $("#node-cloud-import-error-msg").html("" + "Not Data Analytic Microservices Founded");
                    }
                    //console.log(cloudImportListOptions);
                    $('#cloudImportList').append(cloudImportListOptions);
                    $('#cloudImportList a').click(function (evt) {
        ...
        /* microservices end */
        /* damicroservices init */
        $("#clipboard-dialog-damicroservice").show();
        /* damicroservices end */
        /* s4c init */
        $("#cloud-dialog-copy").hide();
        /* s4c end */
        ...
        $("#clipboard-dialog-close").hide();
        $("#clipboard-dialog-copy").hide();
        $("#clipboard-dialog").parent().find(".ui-dialog-titlebar").html("Import Data Analytic MicroService");
        $("#clipboard-dialog-ok").button("disable");
        $("#clipboard-import").keyup(validateImport);
        $("#clipboard-import").on('paste', function () {
            setTimeout(validateImport, 10)
    ...
    function importNodes() {
        ...
        /* s4c init */
        $("#cloud-dialog-copy").hide();
        $("#clipboard-dialog").parent().find(".ui-dialog-titlebar").html("Import nodes");
        /* s4c end */
        $("#clipboard-dialog-ok").button("disable");
        $("#clipboard-import").keyup(validateImport);
        $("#clipboard-import").on('paste', function () {
    ...
    function exportNodes() {
        ...
        /* s4c init */
        $("#cloud-dialog-copy").hide();
        $("#clipboard-dialog").parent().find(".ui-dialog-titlebar").html("Export nodes to clipboard");
        /* s4c end */
        $("#clipboard-dialog-close").hide();
        var selection = RED.view.selection();
        if (selection.nodes) {
    ...
    /* s4c init */
    /*  crea flusso da esportare current flow */
    function createCloudFlowNRangeNodeSet() {
        ...
        flowToExport.push(activeWorkspace);

        var links = [];
        var linkinNodes = nodes.filter(el => {
            return el.type == "link in";
        });

        for (var i = 0; i < linkinNodes.length; i++) {

            links = links.concat(linkinNodes[i].links);
        }


        //controlla i all_nodes flussi linkati che vanno inseriti
        var allFlowNodes = RED.nodes.createCompleteNodeSet(false);
        allLinkoutNodes = allFlowNodes.filter(el => {
            return el.type == "link out";
        });

        for (var i = 0; i < allLinkoutNodes.length; i++) {
            var id = allLinkoutNodes[i].id;



            if (links.indexOf(id) > -1) {
                console.log("flowToExport.push");
                console.log(allLinkoutNodes[i]["z"]);
                flowToExport.push(allLinkoutNodes[i]["z"]);
            }
        }
        ...
        var nodesToExport = [];
        for (var i = 0; i < flowToExport.length; i++) {
            nodesToExport = nodesToExport.concat(RED.nodes.filterNodes({
                z: flowToExport[i]
            }));
        ...
        //il primo flusso  non ha parent flow per essere importato  sia in modalita "current fLow" che in modalit� "parent flow"
        for (var i = 1; i < flowToExport.length; i++) {
            var parentNode = RED.nodes.workspace(flowToExport[i]) || RED.nodes.subflow(flowToExport[i]);
            nodesToExport.unshift(parentNode);
        }


        var nodesToExport = nodesToExport.filter(el => {
            return el.type != "iot-directory-out" && el.type != "iot-directory-in";
        });



        nodesToExport = RED.nodes.createExportableNodeSet(nodesToExport);

        return nodesToExport;

    }
    /* esporta  nodi su sistema s4c */
    function cloudExportNodes() {

        console.log("cloud export nodes")
        if (disabled) {
            return;
        }

        dialogContainer.empty();
        dialogContainer.append($(cloudExportNodesDialog));
        dialogContainer.i18n();


        var natureUrl = "https://servicemap.disit.org/WebAppGrafo/sparql?query=select%20distinct%20substr%28str%28%3Fm%29%2C37%29%20as%20%3FmacroClass%20%7B%20%3Fm%20rdfs%3AsubClassOf%20km4c%3AService.%20filter%20exists%20%7B%3Fc%20rdfs%3AsubClassOf%20%3Fm%7D%20%7D%20order%20by%20%3Fm&format=json";
        var subnatureUrl = "https://servicemap.disit.org/WebAppGrafo/sparql?query=select%20distinct%20substr%28str%28%3Fm%29%2C37%29%20as%20%3FmacroClass%20substr%28str%28%3Fc%29%2C37%29%20as%20%3FsubClass%20%7B%20%3Fm%20rdfs%3AsubClassOf%20km4c%3AService.%20%3Fc%20rdfs%3AsubClassOf%2B%20%3Fm.%20%7D%20order%20by%20%3Fm%20%3Fc&format=json";
        ...
        $.getJSON(natureUrl, function (result) {
            ...
            $('#cloud-data-export-nature').append(natureSelectOptions);
        });
        ...
        $.getJSON(subnatureUrl, function (result) {
            ...
            $('#cloud-data-export-subnature').append(subnatureSelectOptions);
        });


        $('#cloud-data-export-nature').change(function () {
            console.log("change");
            $('#cloud-data-export-subnature option').attr("style", "display:none;");
            $('#cloud-data-export-subnature option[data-nature=default]').attr("style", "display:initial;");
            $('#cloud-data-export-subnature').val('');
            $('#cloud-data-export-subnature option[data-nature=' + $('#cloud-data-export-nature').val() + ']').attr("style", "display:initial;");
        ...
        $("#export-range-group > a").click(function (evt) {
            ...
            if (type === 'export-range-selected') {
                ...
                nodes.unshift(parentNode);
                nodes = RED.nodes.createExportableNodeSet(nodes);*/
                nodes = createCloudFlowNRangeNodeSet();
                cloudData = nodes;
            } else if (type === 'export-range-full') {
                nodes = RED.nodes.createCompleteNodeSet(false);
        ...
        $("#cloud-dialog-copy").hide();
        /* s4c end */
        //$("#export-range-group").parent().hide();
        $("#clipboard-dialog").parent().find(".ui-dialog-titlebar").html("Export nodes S4C");
    ...
    function exportSaveNodes() {
        ...
        dialogContainer.empty();
        dialogContainer.append($(exportSaveNodesDialog));
        dialogContainer.i18n();
        var format = RED.settings.flowFilePretty ? "export-format-full" : "export-format-mini";
        ...
        init: function () {
            ...
            RED.events.on("view:selection-changed", function (selection) {
                if (!selection.nodes) {
                    ...
                    /* s4c init */
                    RED.menu.setDisabled("menu-item-export-cloud", true);
                    /* s4c end */
                } else {
                    ...
                    /* s4c init */
                    RED.menu.setDisabled("menu-item-export-cloud", false);
                    /* s4c end */
                    /* microservices init /
					RED.menu.setDisabled("menu-item-export-cloud",false);
                    /* microservices end */
            ...
            /* s4c end */
            /* s4c init */
            RED.actions.add("core:show-export-cloud-dialog", cloudExportNodes);
            RED.actions.add("core:show-import-cloud-dialog", cloudImportNodes);
            /* s4c end */
            /* microservices init */
            RED.actions.add("core:show-import-microservice", cloudImportMicroService);
            /* microservices end */
            /* damicroservices init */
            RED.actions.add("core:show-import-damicroservice", cloudImportDAMicroService);
            /* damicroservices end */

            RED.actions.add("core:show-export-save-dialog", exportSaveNodes);
...
RED.library = (function () {
    ...
    function loadFlowLibrary() {
        $.getJSON("library/flows", function (data) {
            ...
            if (examples) {
                RED.menu.addItem("menu-item-import", {
