...
if(isset($_GET['token'])) {
    ...
    try{
        ...
        $stmt = $conn->prepare("SELECT * FROM users WHERE token = :v1;");
        $stmt->bindParam(":v1", $_GET['token']);
        $stmt->execute();
        ...
        if ($row) {
            ...
            if ($totalMinutes <= $maxTime) {
                if ($row["statusofregistration"]!="unregistered") {
                    outputHTML($errors="User is already registered.",$code=409);
                }
                else {
                    $things_to_pass = [$row["userid"], $row["mail"], $_GET['token']];
                    outputHTML($errors=null, $code = 200, $params=$things_to_pass);
                }
            }
            else {
                $stmt = $conn->prepare("DELETE FROM users WHERE userid = :v2;");
                $stmt->bindParam(':v2', $userid);
                $stmt->execute();
                $rowCount = $stmt->rowCount();
                if ($rowCount > 0) {
                    outputHTML($errors="Link on mail is expired, please start the procedure again", $code=400);
                } else {
                    error_log("Error trying to remove an user ($userid): user not exists", $code=500);
...
function outputHTML($errors=null, $code=500, $params=null) {
    ...
    $config = [
        ...
        "form_template" =>  $templatesFolder . "/register_user_form_template.html",
    ];
    if ($errors) {
        try{
    ...
    elseif ($params) {
        ...
        $html=str_replace("[[username]]", $params[0], $html);
        $html=str_replace("[[email]]", $params[1], $html);
        $html=str_replace("[[token]]", $params[2], $html);
        echo $html;
    }
    exit();
}
...
function formHTML() {
    ...
    <form action="create_ldap_user.php" method="POST" onsubmit="checkForm(event)">
    <div class="form-group">
    <label for="user">Username:</label>
    <input type="text" class="form-control" id="user" name="user" required value="[[username]]" readonly>
    </div>
    <div class="form-group">
    ...
    <button type="submit" class="btn btn-primary mt-3">Complete your registration</button>
    </form>
    </div>
    </body>
    </html>
