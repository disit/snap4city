 ...
 */
@WebServlet(name = "MakePublic", urlPatterns = {"/make-public"})
public class MakePublic extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            xlogger = new XLogger(MakePublic.class.getName(),cfgParser.getLoggingFolder());
            ...
            xlogger.log(MakePublic.class.getName(), Level.INFO, "request", repos.get(ParserConst.REQUEST).getParameter(ParserConst.REQUEST_RAW));
            if(Const.ERROR == xlogger.getStatus()) {
            ...
            for(int n = 0; n < dataNodes.getLength(); n++) {
                ...
                xlogger.log(MakePublic.class.getName(), Level.FINE, "successfull data built", MessageFormat.format("Successfull build of data: {0}.",new Object[]{builtData.getId()}));
                ...
                if(cfgNode.hasAttribute(ParserConst.CFG_AT_DATA_TRIGGER)) {
                    if (null == data.get(cfgNode.getAttribute(ParserConst.CFG_AT_DATA_TRIGGER)).getValue() || 
                        ...
                        xlogger.log(MakePublic.class.getName(), Level.FINE, "untriggered data", MessageFormat.format("Data found to be untriggered: {0}. It is expected not to be produced in output, nor to be persisted by loaders. It could be employed for cleaning purposes in full updates.",new Object[]{builtData.getId()}));
                    }
                }
                ...
                if(cfgNode.hasAttribute(ParserConst.CFG_AT_DATA_LOAD) && !ParserConst.CFG_RLDRID_VOLATILE.equals(cfgNode.getAttribute(ParserConst.CFG_AT_DATA_LOAD))) {
                    ...
                    if(Const.ERROR == loaders.get(cfgNode.getAttribute(ParserConst.CFG_AT_DATA_LOAD)).getStatus()) {
                        ...
                        xlogger.log(MakePublic.class.getName(), Level.FINE, "successfull data load", MessageFormat.format("Data loaded successfully: {0}", new Object[]{builtData.getId()}));
                    }
                }
            ...
            for(String loaderID: loaders.keySet()) {
                ...
                if(Const.ERROR == loaders.get(loaderID).getStatus()) {
                    ...
                    xlogger.log(MakePublic.class.getName(), Level.FINE, "loader disconnected", MessageFormat.format("Loader {0} disconnected.", new Object[]{loaderID}));
                }
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                xlogger.log(MakePublic.class.getName(), Level.INFO, "response", responseString);
            }
            catch(Exception e) {
            ...
            if(xlogger!=null) xlogger.log(MakePublic.class.getName(), Level.SEVERE, "insert error", e);
