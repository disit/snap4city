 ...
 */
@WebServlet(name = "InsertStaticAttr", urlPatterns = {"/InsertStaticAttr"})
public class InsertStaticAttr extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            xlogger = new XLogger(InsertStaticAttr.class.getName(),cfgParser.getLoggingFolder());
            ...
            xlogger.log(InsertStaticAttr.class.getName(), Level.INFO, "request", repos.get(ParserConst.REQUEST).getParameter(ParserConst.REQUEST_RAW));
            if(Const.ERROR == xlogger.getStatus()) {
            ...
            for(int n = 0; n < dataNodes.getLength(); n++) {
                ...
                xlogger.log(InsertStaticAttr.class.getName(), Level.FINE, "successfull data built", MessageFormat.format("Successfull build of data: {0}.",new Object[]{builtData.getId()}));
                ...
                if(cfgNode.hasAttribute(ParserConst.CFG_AT_DATA_TRIGGER)) {
                    if (null == data.get(cfgNode.getAttribute(ParserConst.CFG_AT_DATA_TRIGGER)).getValue() || 
                        ...
                        xlogger.log(InsertStaticAttr.class.getName(), Level.FINE, "untriggered data", MessageFormat.format("Data found to be untriggered: {0}. It is expected not to be produced in output, nor to be persisted by loaders. It could be employed for cleaning purposes in full updates.",new Object[]{builtData.getId()}));
                    }
                }
                ...
                if(cfgNode.hasAttribute(ParserConst.CFG_AT_OUTPUT_DATA) && builtData.isTriggered()) {
                    try {
                        ...
                        builtData.setOutput(outputCfg);
                        xlogger.log(InsertStaticAttr.class.getName(), Level.WARNING, "Unable to load output configuration. Default configuration will be used.",outputException);
                    }                    
                }
                ...
                if(cfgNode.hasAttribute(ParserConst.CFG_AT_DATA_LOAD) && !ParserConst.CFG_RLDRID_VOLATILE.equals(cfgNode.getAttribute(ParserConst.CFG_AT_DATA_LOAD))) {
                    ...
                    if(Const.ERROR == loaders.get(cfgNode.getAttribute(ParserConst.CFG_AT_DATA_LOAD)).getStatus()) {
                        ...
                        xlogger.log(InsertStaticAttr.class.getName(), Level.FINE, "successfull data load", MessageFormat.format("Data loaded successfully: {0}", new Object[]{builtData.getId()}));
                    }
                }
            ...
            for(String loaderID: loaders.keySet()) {
                ...
                if(Const.ERROR == loaders.get(loaderID).getStatus()) {
                    ...
                    xlogger.log(InsertStaticAttr.class.getName(), Level.FINE, "loader disconnected", MessageFormat.format("Loader {0} disconnected.", new Object[]{loaderID}));
                }
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                xlogger.log(InsertStaticAttr.class.getName(), Level.INFO, "response", responseString);
            }
            catch(Exception e) {
            ...
            if(xlogger!=null) xlogger.log(InsertStaticAttr.class.getName(), Level.SEVERE, "insert error", e);
